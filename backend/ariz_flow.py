...
- 如果矛盾不清晰，帮助用户提炼
- 信息收集完成后，用自然文本输出分析结果，然后【立即调用 ariz_step1_problem 工具】
- system_keywords 必须使用系统级名称（如'热管理系统'、'箱体结构'、'BMS'），不要用具体现象（如'低温续航衰减'、'PTC加热'）
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
...
4. 用自然文本展示组件清单，然后【立即调用 ariz_step2_components 工具】保存
5. 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
...
规则：
- 用自然文本展示接触关系
- 分析完成后【立即调用 ariz_step3_contacts 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
- 【严禁】跳过工具调用。如果你没有调用工具，系统将无法保存分析结果
...
规则：
- 用自然文本列表展示功能模型
- 分析完成后【立即调用 ariz_step4_function 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
- 【严禁】跳过工具调用。如果你没有调用工具，系统将无法保存分析结果
...
规则：
- 用自然文本展示结构分析
- 分析完成后【立即调用 ariz_step5_structure 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示问题清单
- 分析完成后【立即调用 ariz_step6_summary 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示因果链（树状结构）
- 分析完成后【立即调用 ariz_step7_causal 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示矛盾定义和IFR
- 分析完成后【立即调用 ariz_step8_keypoint 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
- 工具调用后系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示方案卡片
- 分析完成后【立即调用 ariz_step9_solution 工具】保存
- 【严禁】先问"请确认"再调工具。必须在同一轮对话中同时完成：输出分析 + 调用工具
...
核心规则（必须遵守）：
- 【最重要】每一步分析完成后，必须在同一轮对话中同时完成：用自然文本输出分析结果 + 调用对应工具保存。绝对不要分开做。
- 【严禁】先输出分析结果、等用户确认后才调用工具。工具调用是你的职责，不需要用户授权。
- 【严禁】在文本中问"请确认""是否正确""对吗"之类的问题。卡片会处理确认流程。
- 工具调用后系统会自动渲染步骤卡片，用户点击卡片上的「确认」按钮进入下一步
    ...
    if step_name == "problem":
        ...
        if primary:
            ...
            if comps:
                ...
                lines.append(f"- 数据库组件（{len(comp_names)}个）：{'、'.join(comp_names)}")
        return "\n".join(lines)
...
def query_components_for_step2(step1_result: dict) -> dict:
    ...
    if not all_systems:
        for kw in keywords:
            # 尝试去掉修饰词，只保留核心词
            for suffix in ["衰减", "效率", "能耗", "问题", "不足"]:
                if suffix in kw:
                    bare = kw.replace(suffix, "")
                    if len(bare) >= 2:
                        systems = search_system(bare)
                        all_systems.extend(systems)
            # 尝试常见的系统别名映射
            alias_map = {
                "PTC": "热管理系统",
    ...
    if not unique_systems:
        return {"error": f"关键词 {keywords} 未匹配到任何系统，请在第2步手动补充组件", "matched_systems": []}
