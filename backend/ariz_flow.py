...
- 如果矛盾不清晰，帮助用户提炼
- 用自然文本输出分析结果，不要用代码块
- 信息收集完成后，用自然文本展示结构化结果，然后调用 ariz_step1_problem 工具保存
...
4. 用自然文本展示组件清单，请用户确认
5. 分析完成后调用 ariz_step2_components 工具保存，传入 supersystem、supersystem_components、system_name、user_added
...
规则：
- 用自然文本展示接触关系，不要用代码块
- 分析完成后调用 ariz_step3_contacts 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
...
- 分析完成后调用 ariz_step4_function 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示结构分析，不要用代码块
- 分析完成后调用 ariz_step5_structure 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示问题清单，不要用代码块
- 分析完成后调用 ariz_step6_summary 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
- 分析完成后调用 ariz_step7_causal 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
- 分析完成后调用 ariz_step8_keypoint 工具保存，系统自动渲染卡片，用户点击「确认」进入下一步
""",
...
规则：
- 用自然文本展示方案卡片，不要用代码块
- 分析完成后调用 ariz_step9_solution 工具保存，系统自动渲染卡片，用户点击「确认」完成
- 生成完整ARIZ分析报告
""",
    }
...
- 步骤卡片由系统自动生成：当你调用工具保存结果后，系统会自动渲染对应的步骤卡片
- 每步的交互流程：分析 → 调用工具保存 → 系统自动渲染卡片 → 用户点击「确认」进入下一步 → 如果用户补充信息则重新分析并再次调用工具
